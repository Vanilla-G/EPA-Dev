...
if [[ "$azure_response" == *"compute"* ]]; then
  ...
  # Check if AWS response is empty or contains "error"
  if [[ -z "$aws_response" || "$aws_response" == *"error"* ]]; then
...
if [[ -z "$S_DOMAIN" || -z "$S_PUBLIC_IP" || -z "$CF_API" || -z "$CF_ZONE_ID" ]]; then
  echo "Error: Required environment variables (S_DOMAIN, S_PUBLIC_IP, CF_API, CF_ZONE_ID) are not set." | tee -a $LOG_FILE
  exit 1
fi
